...
// AI ile üretilen arka plan görsellerini servis et (backend/public/maps/*.png)
...
io.on('connection', (socket) => {
    ...
    // ===================== HEX HARİTA OLAYLARI (YENİ) =====================
    // Mevcut olaylar değişmeden kalır; bunlar yalnızca eklenti niteliğindedir.

    // --- GM HEX HARİTA GÜNCELLEMESİ ---
    // map: oyunculara gönderilen FİLTRELENMİŞ görünüm (gizli hex/nesne/token ve gmNotes çıkarılmış)
    socket.on('gm_hexmap_update', ({ gameCode, map }) => {
        socket.to(gameCode).emit('hexmap_updated', { map });
    });

    // --- OYUNCU TOKEN HAREKETİ (oyuncu -> GM + odadakiler) ---
    // GM yetkili kaynaktır: bu olayı alır, kendi state'inde uygular ve geri yayınlar.
    socket.on('player_token_move', ({ gameCode, tokenId, q, r, playerName }) => {
        socket.to(gameCode).emit('token_moved', { tokenId, q, r, playerName });
    });

    // --- OYUNCU NESNE TOPLAMA (oyuncu -> GM) ---
    socket.on('player_object_pickup', ({ gameCode, objectId, tokenId, playerName }) => {
        socket.to(gameCode).emit('object_pickedup', { objectId, tokenId, playerName });
    });

    // --- LAZER İŞARETÇİSİ (geçici/kaybolan) — GM ve oyuncular paylaşır ---
    socket.on('laser_point', ({ gameCode, points, color }) => {
        socket.to(gameCode).emit('laser_pointed', { points, color });
    });
