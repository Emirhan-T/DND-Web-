...
// GET /api/games/:gameCode/map - Aktif hex haritayı getir (kalıcı kayıt)
router.get('/:gameCode/map', verifyToken, async (req, res) => {
    try {
        const game = await Game.findOne({ gameCode: req.params.gameCode.toUpperCase() });
        if (!game) {
            return res.status(404).json({ message: 'Game not found.' });
        }
        res.status(200).json({ map: game.map || null });
    } catch (error) {
...
// PUT /api/games/:gameCode/map - Hex haritayı kaydet (yalnızca GM)
router.put('/:gameCode/map', verifyToken, async (req, res) => {
    try {
        const game = await Game.findOne({ gameCode: req.params.gameCode.toUpperCase() });
        if (!game) {
            return res.status(404).json({ message: 'Game not found.' });
        }
        // Sadece oyunu kuran GM haritayı değiştirebilir
        if (game.gmId.toString() !== req.user.id) {
            return res.status(403).json({ message: 'Only the GM can modify the map.' });
        }
        game.map = req.body.map;
        game.markModified('map'); // Mixed tipi için değişikliği garantiye al
        await game.save();
        res.status(200).json({ message: 'Map saved.' });
    } catch (error) {
        console.error('Error saving map:', error);
        res.status(500).json({ message: 'Failed to save map.' });
    }
});
