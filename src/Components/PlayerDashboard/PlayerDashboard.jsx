...
import { applyMapJson, mapPixelRect, axialKey } from '../HexMap/hexUtils';
...
const PlayerDashboard = () => {
    ...
    const isBlockedForPlayer = (q, r) => {
        const hex = playerMap.hexes[axialKey(q, r)];
        if (!hex || hex.walkable === false) return true;              // off-map / fog / wall-water-lava terrain
        if (playerMap.mapObjects.some(o => o.q === q && o.r === r && o.blocksMovement)) return true; // wall/tree/door/etc.
        return false;
    };
    const handlePlayerTokenMove = (tokenId, q, r) => {
        if (isBlockedForPlayer(q, r)) { addLog('🚫 Oraya gidemezsin (engelli alan).', false); return; }
