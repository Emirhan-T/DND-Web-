...
import { generateMapFromPrompt, generateBackgroundImage, importMapJson } from '../HexMap/aiMapService';
...
const GMDashboard = () => {
    ...
    const handleTokenMove = (tokenId, q, r) => setTokens(prev => prev.map(t => t.id === tokenId ? { ...t, q, r } : t));
    ...
    const handleGenerateMap = async () => {
        addLog('🤖 Requesting AI map…', true);
        const result = await generateMapFromPrompt(gameCode, aiPrompt, { width: mapMeta.width, height: mapMeta.height });
        if (!result.ok) { addLog(`🤖 ${result.reason}`, true); return; }
        applyImportedMap(result.map);
        addLog('🤖 AI map applied.', false);
    };
    const handleGenerateBackground = async () => {
        addLog('🤖 Requesting AI background…', true);
        const result = await generateBackgroundImage(gameCode, aiPrompt);
        if (!result.ok) { addLog(`🤖 ${result.reason}`, true); return; }
        setBackgroundImageUrl(result.url);
        addLog('🤖 AI background applied.', false);
    };
    const handleSetBackground = () => {
        setBackgroundImageUrl(bgUrlInput.trim() || null);
        addLog(bgUrlInput.trim() ? '🖼️ Background image set.' : '🖼️ Background cleared.', false);
    };
                        ...
                        {bottomTab === 'grid' ? (
                            <div className="hexmap-panel">
                                ...
                                <div className="hexmap-box hexmap-ai-box">
                                    ...
                                    <h5>AI GENERATION</h5>
                                    <div className="hexmap-ai-row">
                                        <input type="text" placeholder="Describe the map / scene…" value={aiPrompt} onChange={(e) => setAiPrompt(e.target.value)} />
                                    </div>
                                    <div className="hexmap-ai-row">
                                        <button type="button" className="hexmap-ai-btn" onClick={handleGenerateMap}>🤖 Generate Map</button>
                                        <button type="button" className="hexmap-ai-btn secondary" onClick={handleGenerateBackground}>🤖 Generate BG</button>
                                    </div>
                                    <p className="hexmap-ai-note">No AI provider connected yet — these call a backend stub. Paste map JSON below to load one now.</p>
