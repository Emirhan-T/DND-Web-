    ...
    if (layer === 'terrain') {
        return (
            <div className="hexmap-terrain-layer" style={{ position: 'absolute', inset: 0 }}>
                ...
                >
                    {hexConfig.visible && Object.entries(hexes).map(([key, h]) => {
                        const [q, r] = key.split(',').map(Number);
                        const c = hexCenter(q, r, size, origin);
                        const def = TERRAINS[h.terrain] || TERRAINS.grass;
                        const blocked = !h.walkable;
                        const hidden = h.visibleToPlayers === false; // only GM receives these
                        return (
                            <polygon
                                key={key}
                                points={hexPolygonPoints(c.x, c.y, size)}
                                fill={def.color}
                                fillOpacity={(hexConfig.opacity ?? 1) * (hidden ? 0.45 : 1)}
                                stroke={blocked ? 'rgba(255,90,90,0.85)' : 'rgba(255,255,255,0.28)'}
                                strokeWidth={blocked ? 2 : 1}
                                strokeDasharray={hidden ? '6,4' : undefined}
                                className={`hexmap-hex ${isHexTool ? 'paintable' : ''}`}
