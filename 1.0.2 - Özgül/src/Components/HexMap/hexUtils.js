...
// Pixel bounds of an axial-rhombus map (q in [0,width), r in [0,height))
function mapBounds(width, height, size) {
    const corners = [
        axialToPixel(0, 0, size),
        axialToPixel(width - 1, 0, size),
        axialToPixel(0, height - 1, size),
        axialToPixel(width - 1, height - 1, size),
    ];
    const xs = corners.map(c => c.x);
    const ys = corners.map(c => c.y);
    return {
        minX: Math.min(...xs) - size, maxX: Math.max(...xs) + size,
        minY: Math.min(...ys) - size, maxY: Math.max(...ys) + size,
    };
}
...
export function mapPixelRect(width, height, size, pad = size) {
    const origin = getRenderOrigin(width, height, size);
    const corners = [
        hexCenter(0, 0, size, origin), hexCenter(width - 1, 0, size, origin),
        hexCenter(0, height - 1, size, origin), hexCenter(width - 1, height - 1, size, origin),
    ];
    const xs = corners.map(c => c.x), ys = corners.map(c => c.y);
    return {
        left: Math.min(...xs) - pad, top: Math.min(...ys) - pad,
